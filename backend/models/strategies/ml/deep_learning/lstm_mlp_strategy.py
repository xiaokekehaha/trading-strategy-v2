...
class LSTMMlPModel(nn.Module):
    ...
    def forward(self, x):
        # 调整输入维度 (batch_size, seq_len, features)
        if len(x.shape) == 2:
            x = x.unsqueeze(1)
...
class LSTMMlPStrategy(BaseDLStrategy):
    def __init__(self,
                 ...
                 batch_size: int = 32):
        super().__init__("LSTM+MLP", int(lookback_period))
        self.hidden_dim = int(hidden_dim)  # 确保是整数
        self.num_layers = int(num_layers)  # 确保是整数
    ...
    def _train_model_impl(self, X: np.ndarray, y: np.ndarray):
        try:
            ...
            logger.error(f"LSTM+MLP模型训练失败: {str(e)}")
            raise